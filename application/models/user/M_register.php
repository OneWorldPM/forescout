            ...
            if ($post['email'] != '' && $post['password'] != '') {
                ...
                if ($customer->num_rows() > 0) { //Check Email or Phone exist with new User 
                    ...
                    $set = array(
                        ...
                        'member_status' => trim($post['attendee_type']),
                        'company_name' => trim($post['company_name']),
                        'number_of_employees' => trim($post['number_of_employees']),
                    ...
                    if ($cust_id > 0) {
                        if ($post['promo_code'] != "") {
                            $promo_code_data = $this->db->get_where("promo_code", array("promo_code" => $post['promo_code']))->row();
                            if (!empty($promo_code_data)) {
                                $flg = 1;
                                if ($promo_code_data->discount >= $flg) {
                                    $qty = $promo_code_data->discount - $flg;
                                    $this->db->update("promo_code", array("discount" => $qty), array("promo_code_id" => $promo_code_data->promo_code_id));
                                }
                            }
                        }
...
//            $this->common->sendEmail($from, $user_data->email, $subject, $message);
    ...
    public function update_location() {
        ...
        if ($cust_id != "") {
            $set = array(
                'location_status' => trim($post['set_location'])
            );
            $this->db->update("customer_master", $set, array('cust_id' => $cust_id));
            return $cust_id;
        } else {
            ...
            if ($post['type'] == "referral_register" && $post['partner_id'] != "") {
                $partner = $this->db->get_where("tbl_partner", array("partner_id" => $post['partner_id']))->row();
                if (!empty($partner)) {
                    $set = array(
                        "register_id" => $register_id,
                        'location_status' => trim($post['set_location']),
                        'partner_id' => trim($post['partner_id']),
